...
### 1. **`top`** `top` 是一个实时显示系统资源使用情况的命令行工具，可以查看进程的内存和 CPU 使用情况。
- 执行命令：
  ```bash
  top
  然后查找进程 ID (PID) 对应的 `a.out` 程序。
  在 `top` 界面中，你可以看到内存使用情况，包括虚拟内存 (`VIRT`)、常驻内存 (`RES`)、共享内存 (`SHR`) 等。
  - `VIRT`：进程使用的虚拟内存总量。
  - `RES`：进程实际使用的物理内存大小。
  - `SHR`：进程共享的内存大小。
### 2. **`ps`**
`ps` 是一个查看进程的工具，你可以通过它查看 `a.out` 程序的内存使用情况。
- 执行命令：
  ```bash
  ps aux | grep a.out
  这个命令会列出所有正在运行的进程，并显示相关信息，包括内存占用。你可以找到 `a.out` 进程的 PID，然后关注 `RSS`（常驻内存）和 `VSZ`（虚拟内存）的值。
  - `RSS`（Resident Set Size）：常驻内存大小，即进程实际占用的物理内存。
  - `VSZ`（Virtual Size）：虚拟内存大小，即进程请求的内存总量。
### 3. **`pmap`**
`pmap` 是一个显示进程内存映射的命令，可以显示每个进程占用的内存详细信息。
- 执行命令：
  ```bash
  pmap <PID>
  你需要先找到 `a.out` 程序的 PID，然后替换 `<PID>`。
  例如：
  ```bash
  pmap 1234
  该命令会显示进程的内存映射，包括每个内存区域的大小、权限、地址等信息。
### 4. **`valgrind`**
`valgrind` 是一个强大的调试工具，虽然它主要用于内存泄漏检测和调试，但它也能提供一些关于内存使用的详细报告。
- 执行命令：
  ```bash
  valgrind --tool=massif ./a.out
  `massif` 是 `valgrind` 的一个工具，用于跟踪进程的堆内存使用。它会生成一个详细的报告，显示程序运行过程中内存的分配情况。
### 5. **`htop`**
`htop` 是 `top` 的一个增强版本，提供了更友好的交互式界面。它可以显示每个进程的内存使用情况，并支持更方便的操作。
- 执行命令：
  ```bash
  htop
  `htop` 会以表格的形式显示系统的进程列表，你可以直观地看到内存和 CPU 的使用情况。按下 `F3` 可以进行进程搜索，找到 `a.out`。
### 6. **`smem`**
`smem` 是一个可以显示进程内存使用情况的工具，比 `ps` 更加详细。它会显示每个进程的共享内存（Shared）、私有内存（Private）、和总内存使用情况。
- 执行命令：
  ```bash
  smem -r
  这个命令会列出所有进程的内存使用情况。
### 总结：
- **`top` 和 `htop`** 是实时监控工具，适合查看系统资源使用情况。
- **`ps` 和 `pmap`** 提供更静态的进程信息，适合查看特定进程的内存占用。
- **`valgrind`** 适用于内存分析，特别是检查内存泄漏。
- **`smem`** 提供详细的内存报告，适用于更精细的内存分析。
